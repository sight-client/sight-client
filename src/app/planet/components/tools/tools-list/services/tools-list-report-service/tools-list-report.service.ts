...
@Injectable()
export class ToolsListReportService {
  constructor(
    // protected $viewerService: ViewerService,
    // private $userDataService: UserDataService,
    private $SetCursorProgressSpinnerService: SetCursorProgressSpinnerService,
  ...
  private async formReport(): Promise<OdsDocument | undefined | null> {
    try {
      ...
      for (const item of this.$toolsListService.drawingStores) {
  ...
  private async getSheetRows(
    ...
    try {
      ...
      const sk: OdsCellValue | undefined = this.$mouseCoordsService?.selectedCrs?.() || undefined;
      const allStoresObj = this.$drawingService.allEntitiesListsLinks;
      ...
      for (const group of allStoresObj[toolName]()) {
        ...
        // Более одной строки
        if (toolName === 'addLine' || toolName === 'addRectangle' || toolName === 'addPolygon') {
    ...
    try {
      ...
      for (const toolObj of toolsPseudoCollections) {
        ...
        if (thisStoreIndexInListsStores !== -1) {
          if (
            this.$toolsListService.drawingStores[thisStoreIndexInListsStores]?.activeObjInStore?.()
          ) {
        ...
        for (const entityObj of toolObj.entitiesList) {
          ...
          if (toolName === 'addMark') {
            ...
            const optForMark = {
              ...cloneDeep(this.$addMarkService.optForMark),
              ...{
                name: entityName,
