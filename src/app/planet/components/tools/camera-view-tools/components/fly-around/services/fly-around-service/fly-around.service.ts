...
export class FlyAroundService {
  ...
  public readonly flyAroundEntitiesList = computed(() =>
    this.$cameraViewToolsService.flyAroundEntitiesList(),
  );
  ...
  public cancelThisTool(): void {
    try {
      ...
      if (this.$cameraViewToolsService.flyAroundEntitiesList().length) {
        this.$cameraViewToolsService.allToolEntitiesCleaning(this.toolName);
      }
  ...
  private setFlyingAround(options: CameraViewToolsOptions = {}): boolean {
    try {
      ...
      if (options?.withPoint || options?.withBillboard) {
        ...
        this.$cameraViewToolsService.addNewEntityToCameraViewToolsLayer(pointEntity); // самоочистится при cancelThisTool()
        this.$cameraViewToolsService.temporalEntitiesList.update((arr) => [...arr, pointEntity]); // самоочистится при cancelThisTool()
      }
      // ------------------------------------------------------------------------- //
