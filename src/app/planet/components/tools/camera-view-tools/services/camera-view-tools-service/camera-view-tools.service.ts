...
export interface CameraViewToolsOptions extends ToolOptions {
...
@Injectable()
export class CameraViewToolsService {
  constructor(
  ...
  public clearCameraViewToolsDataSource(): boolean {
    try {
      const dataSource: Cesium.DataSource | undefined =
        this?.$viewerService.viewer.dataSources?.getByName(
          this.$toolsService.cameraViewToolsLayerName,
        )?.[0];
      if (dataSource) {
