...
@Injectable()
export class DrawMarkFloatingWindowService {
  constructor(
    ...
    effect(() => {
      try {
        if (this.$drawMarkService.drawMarkEntitiesList().length) {
          untracked(() => {
            if (this._validPickedEnttity()) {
              const nowGroupId: string | undefined = this._validPickedEnttity()?.id.split('-')[0];
              const index = this.$drawMarkService
                .drawMarkEntitiesList()
  ...
  // Еще используется в draw-mark-floating-window.html
