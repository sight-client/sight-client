...
export class AddLineService {
  ...
  private readonly linesGroupsCounter = computed<number>(() => this.addLineEntitiesList().length);
  ...
  // Еще используется в add-line.ts
  ...
  // Еще используется в add-line-floating-window.ts
  public readonly addLineEntitiesList = computed(() => this.$drawingService.addLineEntitiesList());
  // Еще используется в add-line.ts
  ...
  // Еще используется в add-line-floating-window.service.ts
  public readonly toolName: DrawingToolName = 'addLine';
  // Используется в add-line.ts
  ...
  // Main-функция инструмента add-line.ts
