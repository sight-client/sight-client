...
@Injectable()
export class AddCircleService {
  // --------------------- Блок для хранения основных состояний сервиса (start) ----------------------- //
  // Еще используется в add-circle-floating-window.ts
  public readonly addCircleEntitiesList = computed(() =>
  ...
  private readonly circleAreaGroupsCounter = computed<number>(
    () => this.addCircleEntitiesList().length,
  );
  ...
  // Еще используется в add-circle-floating-window.service.ts
  public readonly toolName: DrawingToolName = 'addCircle';
  // Используется в add-circle.ts
