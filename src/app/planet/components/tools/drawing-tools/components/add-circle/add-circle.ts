...
@Component({
  ...
  template: `
    <button
      ...
      [matTooltip]="getRusDrawingToolName(this.$addCircleService.toolName)"
      matTooltipPosition="left"
      matTooltipShowDelay="1000"
      matBadge="1"
      matBadgeSize="small"
      [matBadgeHidden]="!this.$addCircleService.isActive()"
      class="tool-panel-button"
      matButton="tonal"
      (mousedown)="this.$addCircleService.buttonHandler($event)"
      [disabled]="!this.$addCircleService.isActive() && $addCircleService.drawingsBlocker()"
    >
      <mat-icon>
...
export class AddCircle implements OnInit, OnDestroy {
  declare public readonly toolName;
  constructor(
    protected readonly $addCircleService: AddCircleService,
  ...
  public getToolName() {
    return this.$addCircleService.toolName;
  }
  // Используются в родителе
  public cancelByEsc(): void {
    this.$addCircleService.cancelThisTool();
  }
}
