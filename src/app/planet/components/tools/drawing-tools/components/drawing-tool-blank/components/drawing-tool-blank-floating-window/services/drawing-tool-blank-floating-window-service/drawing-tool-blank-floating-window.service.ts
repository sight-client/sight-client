...
export class DrawingToolBlankFloatingWindowService {
  constructor(
    ...
    effect(() => {
      try {
        if (this.$drawingToolBlankService.drawingToolBlankList().length) {
          untracked(() => {
            if (this._validPickedEnttity()) {
              const nowGroupId: string | undefined = this._validPickedEnttity()?.id.split('-')[0];
              const index = this.$drawingToolBlankService
                .drawingToolBlankList()
  ...
  // Еще используется в drawing-tool-blank-floating-window.html
