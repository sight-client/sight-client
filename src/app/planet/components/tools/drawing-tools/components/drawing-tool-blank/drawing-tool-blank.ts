...
- флаг наличия хотя бы одной группы сущностей в сторе (плюс, применить его в вычисляемом сигнале "storesAreEmpty" в erase-entity.service.ts);
- ссылка на стор - в объект "_allEntitiesListsLinks";
- при необходимости, условие для обработки импортируемого .kml в методе setEntitiesGroupDefaultEntity (tools-list.service.ts).
...
7. В drawing-tools.ts (или соответствующего родителя) импортировать main-копонент своего инструмента, вставить в html, создать и использовать в хэндлерах соответствующую шаблонную ссылку.
8. При необходимости добавить в tools-floating-windows.ts свои импорты в массив floatingWindowsComponents.
9. При необходимости внести свой компонент в структуру tools-list (обеспечить, например, реактивное переменование сущности, подлет к ней и т.д).
10. При необходимости внести свой компонент в структуру tools-list-kml.service.ts.
11. При необходимости внести свой компонент в структуру tools-list-report.service.ts.
