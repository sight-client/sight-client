...
@Injectable()
export class EraseEntityService {
  constructor(
  ...
  // Еще используется в erase-entity.ts (пока отключено - если кнопка находится в контейнере дефолтной подгруппы и дисэйблится, выглядит плохо; сценарий заменен на алерт)
  ...
  // Еще используется в erase-entity.ts
  ...
  // Используется в erase-entity.ts
  ...
  public startEntitiesCleaning(): boolean {
    try {
      ...
      this.$toolsService.createNewCommonHandler('eraseEntity');
