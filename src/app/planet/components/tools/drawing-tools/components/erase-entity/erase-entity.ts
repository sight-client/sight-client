...
@Component({
  ...
  template: `
    <button
      ...
      matBadge="1"
      matBadgeSize="small"
      [matBadgeHidden]="!this.$eraseEntityService.isActive()"
      class="tool-panel-button"
      matButton="tonal"
      (mousedown)="this.$eraseEntityService.buttonHandler($event)"
      [disabled]="!$eraseEntityService.isActive() && $eraseEntityService.drawingsBlocker()"
    >
      <!-- || $eraseEntityService.storesAreEmpty() === true -->
      <mat-icon>
...
export class EraseEntity implements OnInit, OnDestroy {
  public readonly toolName = 'eraseEntity';
  constructor(
    protected $eraseEntityService: EraseEntityService,
  ...
  public cancelByEsc(): void {
    this.$eraseEntityService.cancelThisTool();
  }
}
