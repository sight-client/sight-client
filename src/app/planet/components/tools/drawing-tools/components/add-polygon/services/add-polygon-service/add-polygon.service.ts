...
@Injectable()
export class AddPolygonService {
  // --------------------- Блок для хранения основных состояний сервиса (start) ----------------------- //
  // Еще используется в add-polygon-floating-window.ts
  public readonly addPolygonEntitiesList = computed(() =>
    this.$drawingService.addPolygonEntitiesList(),
  );
  ...
  constructor(
    ...
    effect(() => {
      try {
        if (this.addPolygonEntitiesList()) {
          untracked(() => {
  ...
  private readonly addPolygonGroupsCounter = computed<number>(
    () => this.addPolygonEntitiesList().length,
  );
  private readonly hasErasedAll = linkedSignal<number, boolean>({
    source: this.addPolygonGroupsCounter,
  ...
  // Еще используется в add-polygon-floating-window.service.ts
  public readonly toolName: DrawingToolName = 'addPolygon';
  // Используется в add-polygon.ts
  public buttonHandler(event: MouseEvent): void {
    ...
    if (event.button === 0) {
      ...
      if (this._drawingHasStarted() === false) {
        ...
        if (this.addPolygonGroupsCounter()) {
      ...
      const finishDrawing = async (
        ...
        try {
          ...
          if (
            ...
            //   }
            // }
            if (polygonEntity?.label && polygonEntity.label.text?.getValue() !== labelText) {
