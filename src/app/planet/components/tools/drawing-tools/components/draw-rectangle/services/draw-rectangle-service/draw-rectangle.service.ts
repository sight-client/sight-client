...
export class DrawRectangleService {
  ...
  // Еще используется в draw-rectangle-floating-window.service.ts
  public readonly toolName: DrawingToolName = 'drawRectangle';
  // Используется в draw-rectangle.ts
      ...
      const finishDrawing = async (
        ...
        try {
          ...
          if (
            ...
            //     }
            //   }
            if (polygonEntity?.label && stringFromProperty(polygonEntity.label.text?.getValue()) !== labelTextMain) {
