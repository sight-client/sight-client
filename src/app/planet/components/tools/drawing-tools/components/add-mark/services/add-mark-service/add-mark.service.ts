...
export class AddMarkService {
  ...
  // Еще используется в add-mark-floating-window.ts
  public readonly addMarkEntitiesList = computed(() => this.$drawingService.addMarkEntitiesList());
  // Еще используется в add-mark.ts
  ...
  private readonly marksGroupsCounter = computed<number>(() => this.addMarkEntitiesList().length);
  ...
  // Еще используется в add-mark-floating-window.service.ts
  public readonly toolName: DrawingToolName = 'addMark';
  // Используется в add-mark.ts
      ...
      const addNewMark = async (movement?: Cesium.ScreenSpaceEventHandler.PositionedEvent) => {
        try {
          ...
          optForPoint.properties = {
            systemCoords: this.$mouseCoordsService.selectedCrs(), // актуализация
            numericId: undefined,
          };
  ...
  // // Используется в add-mark-floating-window.html
  // get nowTerrainName() {
