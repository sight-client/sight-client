...
@Injectable()
export class CircleAreaMeasurementsService {
  // --------------------- Блок для хранения основных состояний сервиса (start) ----------------------- //
  // Еще используется в circle-area-measurements-floating-window.ts
  public readonly circleAreaMeasurementsList = computed(() =>
    this.$measureService.circleAreaMeasurementsList(),
  );
  // Еще используется в circle-area-measurements.ts
  public readonly measuresBlocker = computed(() => this.$toolsService.drawingsBlocker());
  private _isActive = signal<boolean>(false);
  // Еще используется в circle-area-measurements.ts и в сервисе плавающего окна инструмента
  get isActive() {
  ...
  constructor(
    ...
    effect(() => {
      try {
        if (this.circleAreaMeasurementsList()) {
          untracked(() => {
  ...
  private readonly circleAreaGroupsCounter = computed<number>(
    () => this.circleAreaMeasurementsList().length,
  );
  ...
  // Еще используется в circle-area-measurements-floating-window.service.ts
  public readonly toolName: MeasuringToolName = 'circleAreaMeasurements';
  // Используется в circle-area-measurements.ts
