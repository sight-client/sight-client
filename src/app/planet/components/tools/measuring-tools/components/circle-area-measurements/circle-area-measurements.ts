...
@Component({
  selector: 'circle-area-measurements',
  providers: [],
  imports: [MatButtonModule, MatIconModule, MatTooltipModule, MatBadgeModule],
  template: `
    <button
      [style.display]="buttonVisibility() ? 'block' : 'none'"
      [matTooltip]="getRusMeasuringToolName($circleAreaMeasurementsService.toolName)"
      matTooltipShowDelay="1000"
      matTooltipPosition="left"
      matBadge="1"
      matBadgeSize="small"
      [matBadgeHidden]="!$circleAreaMeasurementsService.isActive()"
      class="tool-panel-button"
      matButton="tonal"
      (mousedown)="$circleAreaMeasurementsService.buttonHandler($event)"
      [disabled]="
        !$circleAreaMeasurementsService.isActive() &&
        $circleAreaMeasurementsService.measuresBlocker()
      "
    >
      <mat-icon>
...
export class CircleAreaMeasurements implements OnInit, OnDestroy {
  declare public readonly toolName;
  constructor(
    protected readonly $circleAreaMeasurementsService: CircleAreaMeasurementsService,
  ...
  public getToolName() {
    return this.$circleAreaMeasurementsService.toolName;
  }
