...
@Injectable()
export class LinearMeasurementsService {
  constructor(
  ...
  // Еще используется в linear-measurements-floating-window.service.ts
  ...
  // Еще используется в linear-measurements.ts
  ...
  // Еще используется в effect linear-measurements-floating-window.service (при удалении сущностей инструмента)
  ...
  // Еще используется в linear-measurements.ts (для деактивации кнопки инструмента на правой панели приложения)
  public readonly measuresBlocker = computed(() => this.$toolsService.drawingsBlocker());
  private _isActive = signal<boolean>(false);
  // Еще используется в linear-measurements.ts и linear-measurements-floating-window.service.ts
  get isActive() {
  ...
  // Еще используется в linear-measurements-floating-window.service.ts
  public readonly toolName: MeasuringToolName = 'linearMeasurements';
  // Используется в linear-measurements.ts
  ...
  // Еще используются в linear-measurements-floating-window.html
  get mostDetailed() {
    return this._mostDetailed;
  }

  // Используются в linear-measurements-floating-window.html
  ...
  // Используется в linear-measurements-floating-window.html
  get nowTerrainName() {
  ...
  // При этом видимость полей с информацией о рельефе контролируется this.$linearMeasurementsService.mostDetailed()
  private _recalculationFlag = signal<boolean>(false);
  get recalculationFlag() {
    return this._recalculationFlag;
  }
  // Используется в effect linear-measurements-floating-window.service (при наличии у вновь построенной сущности чанка id === '-most-detailed-')
  // и инпуте шага linear-measurements-floating-window.html (onchange-событие)
  ...
  // Используется в хэндлерах для инпутов linear-measurements-floating-window.html и местном effect.
  ...
  // Используется в хэндлерах инпутов linear-measurements-floating-window.html и местном effect.
