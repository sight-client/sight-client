...
@Injectable()
export class CalculateRectangleFloatingWindowService {
  constructor(
      ...
      try {
        if (this.floatingWindowHasClosed() === true) {
          untracked(() => {
            ...
            if (
              this.$calculateRectangleService.measureHasStarted() ||
              this.$calculateRectangleService.isActive()
            ) {
  ...
  // Еще используется в calculate-rectangle-floating-window.html
  ...
  private _area = computed<number>(() => {
    ...
    if (this._validPickedEnttity() !== undefined) {
      untracked(() => {
        const rectanglePolylinePositions = cartesian3ListFromProperty(
