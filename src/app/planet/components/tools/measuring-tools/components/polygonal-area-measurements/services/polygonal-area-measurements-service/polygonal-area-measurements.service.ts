...
@Injectable()
export class PolygonalAreaMeasurementsService {
  // --------------------- Блок для хранения основных состояний сервиса (start) ----------------------- //
  // Еще используется в polygonal-area-measurements-floating-window.ts
  public readonly polygonalAreaMeasurementsList = computed(() =>
    this.$measureService.polygonalAreaMeasurementsList(),
  );
  // Еще используется в polygonal-area-measurements.ts
  public readonly measuresBlocker = computed(() => this.$toolsService.drawingsBlocker());
  private _isActive = signal<boolean>(false);
  // Еще используется в polygonal-area-measurements.ts и в сервисе плавающего окна инструмента
  get isActive() {
  ...
  constructor(
    ...
    effect(() => {
      try {
        if (this.polygonalAreaMeasurementsList()) {
          untracked(() => {
  ...
  private readonly polygonalAreaMeasurementsGroupsCounter = computed<number>(
    () => this.polygonalAreaMeasurementsList().length,
  );
  private readonly hasErasedAll = linkedSignal<number, boolean>({
    source: this.polygonalAreaMeasurementsGroupsCounter,
  ...
  // Еще используется в polygonal-area-measurements-floating-window.service.ts
  public readonly toolName: MeasuringToolName = 'polygonalAreaMeasurements';
  // Используется в polygonal-area-measurements.ts
  public buttonHandler(event: MouseEvent): void {
    ...
    if (event.button === 0) {
      ...
      if (this._measureHasStarted() === false) {
        ...
        if (this.polygonalAreaMeasurementsGroupsCounter()) {
