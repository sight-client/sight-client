...
| Ярус    | Ширина      | Кнопка / шеврон                                    |
| ------- | ----------- | -------------------------------------------------- |
| Desktop | >1080px     | `--regular-btn-size: 32px`, толщина шеврона ⅓      |
| Laptop  | 768–1080px  | **44px**, толщина шеврона ½                        |
| Compact | 461–767px   | **44px**, толщина ½, `tools-panel` по центру снизу |
| Phone   | ≤460px      | **44px**, толщина ½, столбик высоты и свёртка coords |

Шевроны групп tools: высота как `.tool-panel-button`, толщина `--tool-chevron-thickness` (⅓ кнопки выше 1080; ½ с `laptop` ≤1080). Отступы `margin-left` / `margin-bottom` выводятся из толщины. Toggle sidenav: высота 6× кнопки (на `tablet` ≤767 — 4×), ширина ½ кнопки.
...
1. **Раскладка** — только CSS (миксины / `matchMedia` тех же порогов). Столбик высоты, свёртка coords и старт окон во вкладки — `phoneLayout` ≤460. Центр `tools-panel` — CSS `tablet` ≤767. Overlay sidenav — не ширина: UA mobile и `innerWidth` ≤767 (`wideMobile` ложь).
2. **Виртуальный курсор** — существующий UA-regex телефона/планшета **без** `maxTouchPoints` / `ontouchstart`. Включает: красный `+` в центре холста (это курсор, не «тач»), координаты и постановка точек рисования из центра. Миксин навигации при этом монтируется.

Ноутбук с тачскрином в широком окне: мышь, mixin, без крестика. Узкое окно на ПК: телефонный хром по CSS, курсор мыши если UA не телефон, sidenav остаётся `side`.

Чекбокс и поле «координаты под курсором» **не менять**: «курсор» на телефоне — центр холста.

## Compact 461–767

Кнопки уже 44px (с `laptop` ≤1080). Координаты слева снизу развёрнуты (без шеврона свёртки); высота сверху у меню. `tools-panel` по центру снизу (`left: 194px` остаётся на 768–1080). Sidenav `mode=over` и старт закрыт — только UA mobile при ширине ≤767, не у узкого десктопного окна. На UA mobile свайп по полоске 24px + `safe-area-inset-left` открывает вправо и закрывает влево (порог 48px, сильнее по горизонтали); кнопка-шеврон внутри полоски. Не `touchmove` на `document`. Столбик высоты+coords и свёртка coords — нет.
...
- **Sidenav:** overlay, старт закрыт и backdrop — UA mobile при ширине ≤767. Тот же `drawings-list`. Toggle: высота 6× кнопки (на `tablet` — 4×), ширина ½. Свайп вправо/влево — на UA mobile, не на всякой ширине ≤767.
...
- Верх первой табы и верх первого раскрытого окна — одна линия, чуть ниже шапки (на ≤460 — ниже высоты+шеврона).
- **≤460:** при создании окно сразу `collapsed` во вкладки. Слева — только после разворота вкладки.
...
Блок компас+гироскоп и `+/⌂/−` стоит в углу, где был Cesium-fullscreen. Взаимные `right` и зазор компас↔зум не менять. С `laptop` ≤1080 (кнопки 44px) компас `scale(1.1)`; позиция в `znemz-navigation-mixin.scss`.
