...
1. **Раскладка** — только CSS (миксины / `matchMedia` тех же порогов). Столбик высоты, свёртка coords и старт окон во вкладки — `phoneLayout` ≤460. Центр `tools-panel` — CSS `tablet` ≤767. Overlay sidenav — `tabletLayout` ≤767, в том числе десктопный UA. Шире 767 sidenav `side`, включая `wideMobile`.
...
Ноутбук с тачскрином в широком окне: мышь, mixin, без крестика. Узкое окно на ПК: телефонный хром по CSS, курсор мыши если UA не телефон, sidenav `over` и старт закрыт.

Чекбокс и поле «координаты под курсором» **не менять**: «курсор» на телефоне — центр холста.

## Compact 461–767

Кнопки уже 44px (с `laptop` ≤1080). Координаты слева снизу развёрнуты (без шеврона свёртки); высота сверху у меню. `tools-panel` по центру снизу (`left: 194px` остаётся на 768–1080). Sidenav `mode=over` и старт закрыт на ≤767, включая узкое десктопное окно. Полоска 24px + `safe-area-inset-left` есть на UA mobile и на ≤767; шеврон внутри неё; свайп вправо открывает, влево закрывает (порог 48px, сильнее по горизонтали). Не `touchmove` на `document`. Столбик высоты+coords и свёртка coords — нет.
...
- **Sidenav:** overlay, старт закрыт и backdrop — на ≤767, не только UA mobile. Тот же `drawings-list`. Toggle: высота 6× кнопки (на `tablet` — 4×), ширина ½. Свайп вправо/влево — когда есть полоска (UA mobile или ≤767).
