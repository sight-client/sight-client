...
Sidenav `mode=over`, backdrop, старт закрыт — только UA mobile при ширине ≤767 (`isMobile && !wideMobile()`). Иначе `side`, старт открыт, `disableClose` (в том числе узкое окно на десктопном UA и широкий UA mobile). Полоска `.sight-main-sidenav-edge-swipe` (24px + `safe-area-inset-left`) есть на UA mobile и содержит кнопку-шеврон; свайп вправо открывает, влево закрывает (порог 48px, сильнее по горизонтали). На десктопе кнопка вне полоски. Слушатель не на `document`.
...
**461–767:** те же coords без шеврона свёртки, высота как на laptop, столбика высоты+coords нет. `tools-panel` по центру снизу. Overlay sidenav — по UA, не по этой ширине.
