...
Правая панель: `ToolsPanel` — три группы: drawing, measuring, camera (UI камеры описан в viewer-crs).

Слева: `ToolsList` (список вкладки рисования, подлёт, переименование, импорт/экспорт).
...
Общий ввод с холста: `ToolsService` (`ScreenSpaceEventHandler`, `startToolsService` после mouse-coords). Handler-ы уничтожать на teardown.
...
`drawingToolsNames`: `addMark`, `addLine`, `addRectangle`, `addCircle`, `addPolygon`. Русские подписи — `getRusDrawingToolName`.
...
`EraseEntityService` / `EraseEntity`: удаление из этих сторов; в `storesAreEmpty` должен входить каждый флаг «есть сущности».
...
3. Флаг пустоты + `erase-entity.service.ts`.
4. Ссылка в `_allEntitiesListsLinks`.
5. KML `setEntitiesGroupDefaultEntity`, если импортируется; tools-list; ODS-отчёт, если таблица.
...
| Literal | UI |
|---|---|
| `linearMeasurements` | Дистанция |
| `rectangleAreaMeasurements` | Прямоугольная площадь |
| `circleAreaMeasurements` | Площадь окружности |
| `polygonalAreaMeasurements` | Полигональная площадь |
...
Комментарий в `CameraToolsService`: методы measuring/drawing копировались из общей базы и могут содержать лишнюю логику; «чистить» сразу все типы без одобрения нельзя.

## List, KML, ODS

`ToolsListService` — старт после tools service. Список, подлёт, дефолтные свойства entity из KML.

`ToolsListKmlService` — экспорт/импорт `.kml` / `.kmz`. `CustomPropsFromKml` — JSON-безопасные кастомные поля (`toolName`, id, снимки графики). `billboard.image` и HTML-подобные строки недоверенные; санитайзить (`DOMPurify`, не `javascript:` URL). Скил `sight-client-security`.

`ToolsListReportService` (`providedIn` компонента tools-list): ODS через `odf-kit`. Колонки: имя инструмента, широта, долгота, высота, СК, радиус. В пайплайне отчёта позиции — WGS-84 cartesian, затем `CoordSystems` / `Humanify`. Импорт ODS должен попадать в сторы drawing, а не в параллельную модель.

Скачивание/загрузка: `getMomentName`, `downloadBlob`, `uploadBlob` в `@global/lib/common-global.lib`.

## Floating windows

`FloatingWindowItem`: сигналы `collapsed`, `hidden`, `isActive`, `top`, опционально `right`. Стопка сверху ~33px + высота шапки; clamp по `innerHeight - 300`.
