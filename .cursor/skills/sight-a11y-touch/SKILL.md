...
description: Use when changing keyboard access, aria attributes, focus, tooltips, touch handlers, mobile layout, or DeviceService behavior in Sight.
---
...
Есть поддержка тача (`DeviceService.isMobile`). Иконки панели часто без видимого текста — нужен доступный имя.

## Клавиатура и ARIA

- У кнопки с одной иконкой: `aria-label` (как `user-menu.html`) **или** осмысленный `matTooltip`, не пустой `matTooltip="Test"`.
...
- Цвет сущностей на карте не должен быть единственным носителем смысла (есть имя в tools-list).

## Тач / mobile

- Детектор: `DeviceService` (`userAgent` + `maxTouchPoints`). Не плодить второй `isMobile`.
