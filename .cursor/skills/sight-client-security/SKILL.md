...
- Импорт KML/KMZ (`tools-list-kml.service.ts`): не выполнять скрипты из файла, не подставлять произвольный URL в `billboard.image` без проверки (data:/https, не `javascript:`).
...
- Любая «авторизация» в UI (`account-features`) не даёт серверной защиты. Не обещать безопасное хранение паролей.
